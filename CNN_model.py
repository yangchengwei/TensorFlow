...
class CNN_model(object):
    ...
    def model(self, input_tensor, scope='model'):
        with tf.variable_scope(scope, reuse = tf.AUTO_REUSE):
            out = conv2d( input_tensor, self.inputChannel, self.numOfKernel1, scope='conv1', k=2)
            out = conv2d( out, self.numOfKernel1, self.numOfKernel2, scope='conv2', k=2, batchNorm=True)
            out = conv2d( out, self.numOfKernel2, self.numOfKernel3, scope='conv3', k=2)
            out = conv2d( out, self.numOfKernel3, self.numOfKernel4, scope='conv4', batchNorm=True)
            out = conv2d( out, self.numOfKernel4, self.numOfKernel4, scope='conv5')
            out = conv2d_transpose( out, self.numOfKernel4, self.numOfKernel3, scope='deconv4', k=1)
            out = conv2d_transpose( out, self.numOfKernel3, self.numOfKernel2, scope='deconv3')
            out = conv2d_transpose( out, self.numOfKernel2, self.numOfKernel1, scope='deconv2')
            out = conv2d_transpose( out, self.numOfKernel1, self.outputChannel, scope='deconv1', activated=False)
    ...
    def build_model(self):
        ...
        self.loss = tf.nn.l2_loss(tf.abs(self.GT-self.PD))
    ...
    def train(self):
        ...
        for epoch in range(1, self.numOfEpochs+1):
            ...
            while (len(self.readList) != 0):
                trainX, trainY, _ = self.read_image(self.batchSize)
